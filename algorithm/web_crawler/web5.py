...
class Acad:
    ...
    def SavetoDB(self):
        op = "INSERT INTO life VALUES ('%d', '%s', '%s', '%f', '%f', '%f', '%f', '%f', '%f')" % (self.id, self.name, self.country, self.overall, self.teaching, self.research, self.citations, self.industry, self.outlook)
        cursor.execute(op)
