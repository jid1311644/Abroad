...
class Acad:
    ...
    def SavetoDB(self):
        op = "INSERT INTO physical VALUES ('%d', '%s', '%s', '%f', '%f', '%f', '%f', '%f', '%f')" % (self.id, self.name, self.country, self.overall, self.teaching, self.research, self.citations, self.industry, self.outlook)
        cursor.execute(op)

url = 'http://rankings.betteredu.net/THE/2017-2018/physical-sciences.html'
